...
describe('preset furniture facing', () => {
  ...
  it('orients cuisine and bathroom storage into the room', () => {
    ...
    expect(bath['bath-shower']).toBe(270)
    expect(bath['bath-sink']).toBe(270)
    expect(bath['bath-mirror']).toBe(270)
    expect(bath['bath-cabinet']).toBe(270)
    expect(bath['bath-washer']).toBe(0)
    expect(bath['bath-dryer']).toBe(0)
  })
})
